...
public:
   ...
   * huge is checked against allowed_flags separately from the entry itself:
   * masking the query (huge & allowed_flags) would collapse to none at levels
   * that don't support huge, and has_flag(none) is vacuously true.
   **/
  static bool is_page(page_entry_t entry) noexcept {
    const bool is_huge  = util::has_flag(allowed_flags, Pflag::huge) && entry.has_flag(Pflag::huge);
  ...
  auto begin() noexcept { return tbl_.begin(); }
  auto end() noexcept { return tbl_.end(); }
  auto begin() const noexcept { return tbl_.begin(); }
  auto end() const noexcept { return tbl_.end(); }
...
public:
  ...
  ssize_t purge_unused() {
    ...
    for (page_entry_t& ent : tbl_) {
      if (is_page_dir(ent)) {
        ...
        // Recurse first: a sub-directory can become empty only as a result
        // of purging its own children, so it must be re-checked for
        // emptiness after descending, not just before.
        auto sub_bytes = dir->purge_unused();
        if (dir->is_empty()) {
          delete dir;
          ent = {};
        } else {
  ...
  Map map_entry_r(page_entry_t& ent, Map req) {
    ...
    if (is_unmap) {
      ...
      // Only the whole entry can be dropped in one shot when req.lin sits
      // exactly at this entry's start; otherwise the request only wants the
      // tail of this entry's span, and reporting the full page_size as
      // consumed here would make the caller's loop skip past the untouched
      // lead-in bytes on its next iteration, leaking them as still mapped.
      const bool covers_whole_entry = page_size.is_aligned(req.lin) and req.size >= page_size;
      if (is_page_dir(ent)) {
        auto* pdir = page_dir(ent);
        if (covers_whole_entry)
          delete pdir;
        else
  ...
  Map map_r(Map req) {
    ...
    for (auto i = tbl_.begin() + indexof(req.lin); i != tbl_.end(); i++) {
      ...
      // HACK: normalize to req.attrs (the virtual/requested capability) before
      // merging, since raw entry attrs also carry hardware/structural bits
      // (e.g. huge) that vary by chosen page size even for one uniform request.
      // TODO: separate virtual attrs from hardware layout properly instead of
      // patching it at the merge site.
      Map entry_res = map_entry_r(*ent, sub);
      if (entry_res) entry_res.attrs = req.attrs;
      res = Map::merge_maps(res, entry_res);
      if (!res) return res;
