...
/** Tally mapped pages under a leaf page table, by page size **/
template <typename Arch, typename Arch::Flags AllowedFlags>
Summary summary(os::mem::PageTable<Arch, AllowedFlags>& tbl) {
  Summary sum;
  for (auto& ent : tbl)
    if (tbl.is_page(ent) && ent.has_flag(Flags::present))
      sum.add_page(tbl.page_size.bytes());
  return sum;
}

/** Tally mapped pages and page directories under dir, by page size **/
template <typename Arch, page_size_t Psz, typename Sub, typename Arch::Flags AllowedFlags>
Summary summary(os::mem::PageDirectory<Arch, Psz, Sub, AllowedFlags>& dir) {
  Summary sum;
  for (auto& ent : dir) {
    if (dir.is_page(ent)) {
      if (ent.has_flag(Flags::present)) sum.add_page(dir.page_size.bytes());
      continue;
    }
    if (dir.is_page_dir(ent)) {
      sum.add_dir(dir.page_size);
      sum += summary(*dir.page_dir(ent));
    }
  }
  return sum;
}
